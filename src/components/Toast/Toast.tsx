...
const baseStyles =
  'font-medium rounded rounded py-3 drop-shadow-md w-fit pl-3 pr-5 z-100';

const typeStyles = {
  success: { contained: 'bg-success-100', outlined: '' },
  error: { contained: 'bg-error-100', outlined: '' },
  info: { contained: 'bg-info-100', outlined: '' },
  warning: { contained: 'bg-warning-100', outlined: '' },
};
...
const borderBaseStyles = 'w-30 h-38 border-r-2 pr-2 mr-2';
...
export function Toast({
  ...
  return (
    <div className={ToastClass} {...props}>
      <div className="flex">
        {variant === 'contained' ? (
          <div className={clsx(borderBaseStyles, borderStyles[type])}>
            {iconStyles[type]}
          </div>
        ) : (
          <div></div>
